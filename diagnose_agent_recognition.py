...
def dump_pair_debug(env, a_idx, b_idx, a_obs, tag="", anchor_index=0, outdir=None):
    ...
    os.makedirs(os.path.join(outdir, anchor_index), exist_ok=True)
    rgb_path   = os.path.join(outdir, anchor_index, stem + "_rgb.png")
    depth_path = os.path.join(outdir, anchor_index, stem + "_depth.png")
    meta_path  = os.path.join(outdir, anchor_index, stem + "_cam.json")
...
def run():
    ...
    parser.add_argument("--debug_store", "-d", type=str, default="debug_store")
    parser.add_argument("--result_file", "-r", type=str, default="result_file.json")
    ...
    for anchor_index, a_anchor in enumerate(positions_to_test):
        for a_idx in range(args.num_agents):
            for b_idx in range(args.num_agents):
                ...
                dump_pair_debug(env, a_idx, b_idx, a_obs, tag=f"A{a_idx}_vs_B{b_idx}", anchor_index=anchor_index, outdir=args.debug_store)
    ...
    report_path = os.path.join(args.output_dir, args.result_file)
